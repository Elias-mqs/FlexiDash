...
export function UpdateUserModal({ isOpen, onClose, activeModal }: UpdateUserModalProps) {
  const { data: userAccessList } = useQuery<GetListAccessProps>({
    queryKey: ['user', 'get-access', 'update-modal', activeModal],
    queryFn: async () => {
      const dataCrypt = FormsCrypt.dataCrypt(activeModal)
      const res = await api.post('system/user/update-user/get-list-access', dataCrypt)

      console.log('=========================chamouuuu =================', res.data)
      return res.data
    },
  ...
  const idLists = {
    modules: userAccessList.modules.map((module) => module.id),
    routines: userAccessList.routines.map((routine) => routine.id),
    resources: userAccessList.resources.map((resource) => resource.id),
  }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="6xl">
      <ModalOverlay />
      <ModalContent mx={4}>
        <ModalHeader pl={16}>Atualizar dados do usuário</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <PrepareModal idLists={idLists} userDataUpdate={activeModal} />
        </ModalBody>
...
function PrepareModal({
  ...
  idLists: { modules: number[]; routines: number[]; resources: number[] }
  userDataUpdate: ListUsersProps
}) {
  const toast = useToast()

  const [activeIndex, setActiveIndex] = useState<number>(0) // Controla qual lista está ativa (Módulos, Rotinas, ou Recursos)
  const [selectedModules, setSelectedModules] = useState<number[]>(idLists.modules) // Módulos selecionados
  const [selectedRoutines, setSelectedRoutines] = useState<number[]>(idLists.routines) // Rotinas selecionadas

  const [selectedResources, setSelectedResources] = useState<number[]>(idLists.resources) // Estado para recursos selecionados
  ...
  const { handleSubmit, control } = useForm({
    ...
    defaultValues: { name: userDataUpdate.nome, email: userDataUpdate.email, username: userDataUpdate.usuario },
  })
