...
import { createInv, findInventByCod } from '@/utils/database/repositories/inventory/invDocuments'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (
      verifyDoc?.doc_codInv !== dataInvent.doc_codInv &&
      verifyDoc?.doc_armaz !== dataInvent.doc_armaz &&
      verifyDoc?.doc_inicio !== dataInvent.doc_inicio
    ) {
